...
class _ServiceBookingScreenState extends State<ServiceBookingScreen> {
  ...
  void _updateLocalSchedules() {
    // Logic to update local schedules if needed
  }
  ...
  List<String> _calculateAvailableStartTimes() {
    ...
    DateTime toDT(String timeStr) => DateFormat("HH:mm:ss").parse(timeStr);
    String fromDT(DateTime dt) => DateFormat("HH:mm:ss").format(dt);

    final workStart = toDT(workStartStr);
    final workEnd = toDT(workEndStr);
    final serviceDuration =
        int.tryParse(_selectedService?.duration ?? '30') ?? 30;
    ...
    for (final busy in busySchedules) {
      DateTime busyStart = toDT(busy.startTime);
      DateTime busyEnd = toDT(busy.endTime);
    ...
    for (final gap in gaps) {
      ...
      if (gapDuration >= 120) {
        ...
        while (current
            ...
            current
                .add(Duration(minutes: serviceDuration))
                .isAtSameMomentAs(gapEnd)) {
          availableSlots.add(fromDT(current).substring(0, 5));
  ...
  void _applyAvailableSchedules(List<StaffScheduleEntity> schedules) {
    setState(() {
      ...
      for (final s in _staffSchedules.where(
        (item) => item.familySchedule != null,
      )) {
  ...
  void _submit() {
    ...
    if (_selectedService == null) {
      ScaffoldMessenger.of(
        context,
      ).showSnackBar(const SnackBar(content: Text('Vui lòng chọn dịch vụ')));
      return;
    }
                    ...
                    decoration: InputDecoration(
                      ...
                      suffixIcon: IconButton(
                        ...
                        onPressed: isSearchingPhone
                            ...
                            : () async {
                                setModalState(() => isSearchingPhone = true);
                                await _searchCustomerByPhone(
                                  phoneSearchController.text,
                                );
                                if (mounted) {
                                  setModalState(() => isSearchingPhone = false);
                                }

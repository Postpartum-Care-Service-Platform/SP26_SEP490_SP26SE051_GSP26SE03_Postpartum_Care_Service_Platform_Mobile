...
class CreateFeedbackSheet extends StatefulWidget {
  ...
  static void show(
    ...
    try {
      familyScheduleBloc = context.read<FamilyScheduleBloc>();
    } catch (_) {}

    AmenityBloc? amenityBloc;
    try {
      amenityBloc = context.read<AmenityBloc>();
    } catch (_) {}
    ...
    showModalBottomSheet(
      ...
      builder: (sheetContext) => MultiBlocProvider(
        providers: [
          ...
          if (amenityBloc != null) BlocProvider.value(value: amenityBloc),
        ],
...
class _CreateFeedbackSheetState extends State<CreateFeedbackSheet> {
  ...
  void _autoSelectInitialType(List<FeedbackTypeEntity> types) {
    ...
    if (widget.initialSchedule != null) {
      final serviceType = types.cast<FeedbackTypeEntity?>().firstWhere((t) {
      ...
      final amenityType = types.cast<FeedbackTypeEntity?>().firstWhere((t) {
        final name = t?.name.toLowerCase() ?? '';
        return name.contains('tiện ích') || name.contains('amenity');
      }, orElse: () => null);
      if (amenityType != null) {
  ...
  void initState() {
    ...
    if (widget.initialSchedule != null) {
      ...
      _selectedScheduleDate =
          widget.initialDate ?? widget.initialSchedule!.workDate;
    } else if (widget.initialAmenityTicket != null) {
  ...
  Future<void> _pickScheduleDate() async {
    ...
    if (authBloc != null && authBloc.state is AuthCurrentAccountLoaded) {
      final nowPackage =
          (authBloc.state as AuthCurrentAccountLoaded).account.nowPackage;
    ...
    familyScheduleBloc.add(FamilyScheduleLoadByDateRequested(pickedKey));
  }

  void _showSchedulePickerDrawer(
  ...
  void _showAmenityTicketPickerDrawer(
  ...
  void _handleSubmit() {
    ...
    if (_requiresAmenityTicketId && _amenityTicketId == null) {
      AppToast.showError(
        context,
        message: 'Vui lòng chọn AmenityTicket hoàn thành',
      );
      return;
    }
